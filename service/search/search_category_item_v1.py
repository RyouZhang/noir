...
class SearchCategoryItemV1(SearchApiHandler):
    def __init__(self):
    ...
    async def process(self, args, context=None):
        if args.get('cat_id') < 0 and args.get('root_cat_id') < 0:
...
async def rewrite_func(api, args, context):
    root_cat_id = args.get('root_cat_id', -2)
    if root_cat_id == 1002:
        return '/api/search/category/job/v1', args, context
    return api, args, context


router.register_api_rewrite(
    '/api/search/category/item/v1',
    rewrite_func = rewrite_func
)

router.register_api_handler(
    '/api/search/category/item/v1',
    SearchCategoryItemV1(), 
